...
\end{verbatim}
\begin{alltt}
\hlkwd{write}\hlstd{(jsonString,} \hlkwc{file}\hlstd{=}\hlstr{"../q4_newsletter_signup/out/overallSignupRates.json"}\hlstd{)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
Run G-Test, which is a test of independence (just like the Chi-Square test). The null hypothesis is that the two variables are independent, which is rejected if the p-value is smaller than a chosen significance level $\alpha$. Usually one uses $\alpha=5\%$ but with such a large sample size I would opt for $\alpha=0.001$, which means that if we decide to reject the null hypothesis and assume the variables are dependent (i.e. in this case this means one experiment is better than the other), we might be wrong for $\alpha \times n=1064$ observations.
