...
\hlkwd{load}\hlstd{(}\hlstr{"../03_train_model/rfModelData.RData"}\hlstd{)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\hlstd{inTraining} \hlkwb{<-} \hlkwd{createDataPartition}\hlstd{(rfModelData}\hlopt{$}\hlstd{is_delayed,} \hlkwc{times}\hlstd{=}\hlnum{1}\hlstd{,} \hlkwc{p} \hlstd{=} \hlnum{.05}\hlstd{,} \hlkwc{list} \hlstd{=} \hlnum{FALSE}\hlstd{)}
...
\hlstd{delayRF360k} \hlkwb{<-} \hlkwd{randomForest}\hlstd{(is_delayed} \hlopt{~} \hlstd{.,} \hlkwc{data}\hlstd{=rfModelData,} \hlkwc{subset}\hlstd{=inTraining,}
