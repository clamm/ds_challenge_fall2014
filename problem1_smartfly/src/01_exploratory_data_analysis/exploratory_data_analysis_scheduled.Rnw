...
pFactorLevels <- sapply(factorNames, FUN=function(df, x) { levels(df[,x]) }, df=predictDataTyped)
@

<<load-train-levels, include=FALSE>>=
load("../01_exploratory_data_analysis/trainDataTyped.Rdata")
# contains trainDataTyped, factorNames and factorLevels
...
haveMissingLevels <- union(which(nbTMissingLevels!=0), which(nbPMissingLevels!=0))
suitable <- intersect(which(nbLevels<300), haveMissingLevels)
overview <- list()
