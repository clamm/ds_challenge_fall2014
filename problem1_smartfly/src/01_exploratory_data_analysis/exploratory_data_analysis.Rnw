...
delayed_props <- lapply(factor_idx, FUN=delayed_proportion_per_group)
names(delayed_props) <- variableNames$name[factor_idx]
@
...
suitable <- intersect(factor_idx, which(number_of_levels < 20))
